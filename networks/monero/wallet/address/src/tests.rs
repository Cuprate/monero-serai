...
fn featured_vectors() {
  ...
  for vector in vectors {
    ...
    let spend =
      decompress_point(CompressedEdwardsY(hex::decode(vector.spend).unwrap().try_into().unwrap()))
        .unwrap();
    let view =
      decompress_point(CompressedEdwardsY(hex::decode(vector.view).unwrap().try_into().unwrap()))
        .unwrap();
